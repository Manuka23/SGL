...
Public Class frmRotacionPotrerosIngreso
    ...
    Private ExistenPotreros As Boolean = False
    ...
    Private Sub frmRotacionPotrerosIngreso_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub BuscarPastoreos()
        ...
        Try
            ...
            While rdr.Read()
                ...
                dgvRP_Potreros.Rows.Add(rdr("PotreroCod"), PotreroSuperficie, MpastCobertura, dias_disp, Pastoreo, Ensilaje, PastoreoAcum, Potrerotipo, rdr("UltimoPastoreo"))
    ...
    Private Sub TrazaLineaTarjet()
        If ExistenPotreros = False Then Exit Sub
    ...
    Private Sub btnImprime_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnImprime.Click
        If grpIngresoDatos.Visible = True Then
    ...
    Private Sub tabPASTO_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tabGrafMapa.SelectedIndexChanged
        'Si cambia a la pestaña del Mapa, ahi se carga el MAPA.
        If tabGrafMapa.TabPages(tabGrafMapa.SelectedIndex).Name.Contains("2") Then BuscarArchivoMapa(True)
    End Sub
    ...
    Private Sub ValidaSizeChartMes()
        If grpIngresoDatos.Visible = True Then
            ...
            tabGrafMapa.Left = grpIngresoDatos.Left
            tabGrafMapa.Width = grpIngresoDatos.Width
    ...
    Private Sub dgvRP_Potreros_EditingControlShowing(sender As Object, e As DataGridViewEditingControlShowingEventArgs) Handles dgvRP_Potreros.EditingControlShowing
        Dim col As Integer = dgvRP_Potreros.CurrentCell.ColumnIndex
    ...
    Private Sub ValidaTxtRetiro(ByVal sendere As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs)
        Dim columna As Integer = dgvRP_Potreros.CurrentCell.ColumnIndex
        Dim linea As Integer = dgvRP_Potreros.CurrentCell.RowIndex
        ...
        End If
    End Sub
    Private cancelIt As Boolean = True
    Private Sub dgvRP_Potreros_CellValidating(sender As Object, e As DataGridViewCellValidatingEventArgs) Handles dgvRP_Potreros.CellValidating

        Dim headerText As String = dgvRP_Potreros.Columns(e.ColumnIndex).HeaderText

        If headerText IsNot "Pastoreo (Ha/Día)" And headerText IsNot "Ensilaje (Ha/Día)" Then
            Exit Sub
        End If

        Dim rowIdx As Integer = dgvRP_Potreros.CurrentCell.RowIndex
        Dim ValorPastoreo As String = "0"
        Dim ValorEnsilaje As String = "0"
        If headerText Is "Pastoreo (Ha/Día)" Then
            ValorEnsilaje = dgvRP_Potreros.Rows(rowIdx).Cells("EnsilajeHa").Value
        End If
        If headerText Is "Ensilaje (Ha/Día)" Then
            ValorPastoreo = dgvRP_Potreros.Rows(rowIdx).Cells("PastoreoHa").Value
        End If

        Dim ValorPotreroHa As String = dgvRP_Potreros.Rows(rowIdx).Cells("PotreroHa").Value
        Dim valorIngresado As String = e.FormattedValue.ToString
        If valorIngresado IsNot "" And cancelIt Then
            Dim sumaDatos As Decimal = Convert.ToDecimal(IIf(ValorPastoreo = "", 0, ValorPastoreo)) + Convert.ToDecimal(IIf(ValorEnsilaje = "", 0, ValorEnsilaje)) + Convert.ToDecimal(valorIngresado)
            If sumaDatos > Convert.ToDecimal(ValorPotreroHa) Then
                MsgBox("El valor ingresado NO puede ser mayor a las Hectareas del Potrero", MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "ERROR")
                e.Cancel = True
                dgvRP_Potreros.EndEdit()
                cancelIt = False
            End If
        ElseIf (cancelIt = False) Then
            e.Cancel = True
        End If
    End Sub

    Private Sub dgvRP_Potreros_CellBeginEdit(sender As Object, e As DataGridViewCellCancelEventArgs) Handles dgvRP_Potreros.CellBeginEdit
        cancelIt = True
    End Sub

    Private Sub dgvRP_Potreros_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles dgvRP_Potreros.CellEndEdit
    End Sub
End Class