...
Public Class frmBastonV2Conteo
    ...
    Private Formulario As String = "frmConteo"
    Public lvBASTON As ListView

    Public ContDiioVal As Integer
    Public Reprocesa As Integer
    ...
    Private QuitarOrigenExcel As Boolean = False     'para los cierres mensuales
    ...
    ''
    Dim ItemsAllflex As New List(Of ListViewItem)
    ...
    Dim ItemsXRS As New List(Of ListViewItem)
    Dim ItemsXRS2 As New List(Of ListViewItem) 'contendra el listado de diios de las sesiones seleccionadas
    ...
    Public Sub InicilizaBarraEstado(ByVal NroMax As Integer)
                ...
                If EsFinalRespuestaXRS(Buffer) Then
                    ...
                    Select Case OperacionXRS
                        ...
                        Case "DIIOS"
                            ...
                            Dim BarraEstadoVal As Integer = IndXRS - SaveIndXRS
                            ActualizaBarraEstado(BarraEstadoVal)
                ...
                If EsFinalRespuestaXRS(Buffer) Then
                    ...
                    If ComandoXRS = "CantidadDiiosSession" Then                      'Para saber la cantidad de diios en cada sesion
                        If CadenaXRS.ToString.Trim.Contains("/") = False Then
                            ...
                            Dim dias_val As Integer = DiasHabiles(Param1_CentroCod, "CONTEO")

                            'If ValidaTipoCentro > 0 Then dias_val = 30
                            ...
                            If NumeroDiios <> "" Then                                      'Rellena la grilla de sesiones 
                                ...
                                Dim fec_ As DateTime = DateTime.Parse(ArraySesionesXRS2(contIndexSesion)) 'contadorSesion
                                'Dim fec_text As String = Format(ArraySesionesXRS2(contIndexSesion), "MM/dd/yyyy")
                    ...
                    If ComandoXRS = "ListaDiios" Then                                   'Rellena la grilla de Detalle Diios 
                        ...
                        If DIIOSXR2 <> "[]" Then
                            ...
                            If DIIOSXR2 <> "[F1AEID]" And DIIOSXR2 <> "[F0AVID]" And DIIOSXR2 <> "[RDDDate]" And DIIOSXR2 <> "[RTATime]" And DIIOSXR2 <> "[F1AEIDF0A]" Then
    ...
    Private Sub ConseguirListadoDIIOsXRS()
        ...
        If lblTotDiios.Text <> "0" Then
            ...
            If lblRepetidos.Text <> "0" Then btnEliminaRepetidos.Enabled = True
        End If
        fnAuditoria.InsertDetalle(AuditId, Param1_CentroCod, lvXRS_DETALLE, "Baston XRS", SerialBaston, NombreSesion)
    End Sub
    ...
    Private Sub MostrarYValidarDatosAImportar()
        ...
        Try
            ...
            Dim str_rep As String
            Dim lin As Integer
            Dim DIIOFechaXls As Date = Date.Now
            ...
            Do While Procesa
                ...
                If Hoja.Cells(lin, 2).value = Nothing Then
                    Dim hora As Date = Now.ToString("yyyy-MM-dd")
                    DIIOFechaXls = hora
                Else
                ...
                item.SubItems.Add(Format(DIIOFechaXls, "yyyy-MM-dd"))
    ...
    Private Sub lvXRS_SESIONES_ItemChecked(sender As System.Object, e As System.Windows.Forms.ItemCheckedEventArgs) Handles lvXRS_SESIONES.ItemChecked
        ...
        If tbast_ = 4 Then
            ...
            If e.Item.Checked = True Then
                ...
                fnConteoGallagher.ProcesarDiiosAgrident(num, cboPuertos.Text, "Bastoneo")
        ...
        If tbast_ = 3 Then
            ...
            If e.Item.Checked = True Then
                ...
                InicilizaBarraEstado(NumeroDiios)
                lblTotDiios.Text = NumeroDiios + totaldios
                fnAuditoria.InsertDetalle(AuditId, Param1_CentroCod, lvXRS_DETALLE, "Baston Gallagher HR5", "0", NombreSesion)
                Dim puerto As String = cboPuertos.Text
                Dim STR As String = puerto
                Dim Delimitador_A As String = "("
                Dim Delimitador_B As String = ")"
                ...
                fnConteoGallagher.ProcesarDiiosGallagher(num, cboPuertos.Text, "Bastoneo")
            Else
                Dim i As Integer
                Dim cont As Integer
                Dim contrep As Integer
    ...
    Private Sub btnBastonLee_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBastonLee.Click
        ...
        If tbast_ = 0 Then
            TipoLector = "Baston ALLFLEX"
        End If
        If tbast_ = 1 Then
            TipoLector = "Baston XRS"
        End If
        If tbast_ = 2 Then
            TipoLector = "Baston XRS2"
        End If
        If tbast_ = 3 Then
            TipoLector = "Gallagher HR4-5"
        End If
        If tbast_ = 4 Then
            TipoLector = "Agrident"
        End If
        fnDatosAuditoria.Tipocarga = TipoLector
        fnDatosAuditoria.CentroCod = Param1_CentroCod
        AuditId = fnAuditoria.InsertSesion(fnDatosAuditoria, Formulario)

        If AuditId = 0 Then
            MsgBox(fnAuditoria.RetMsg, MsgBoxStyle.Exclamation + MsgBoxStyle.OkOnly, "Error tabla Auditoria.")
            Exit Sub
        End If
        ...
        Try
            ...
            If tbast_ = 3 Then
                ...
                lvXRS_DETALLE.Items.Clear()
                ''''''''''''''''''''''''''''''''''''''
                fnConteoGallagher.ProcesarSesionesGallagher("Bastoneo")
                lblMSG.Text = "AHORA PUEDE LEER EL BASTON"
                pnlMSG.BackColor = Color.ForestGreen
                fnConteoGallagher.CerrarCom()
            ...
            If tbast_ = 4 Then
                ...
                lvXRS_DETALLE.Items.Clear()
                ''''''''''''''''''''''''''''''''''''''
                fnConteoGallagher.ProcesarSesionesAgrident("Bastoneo", com)
                lblMSG.Text = "AHORA PUEDE LEER EL BASTON"
                pnlMSG.BackColor = Color.ForestGreen
                fnConteoGallagher.CerrarCom()
    ...
    Private Sub btnProcesar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcesar.Click
        ...
        frmComparaBastonConteo.lvGanado.Items.Clear()
        frmComparaBastonConteo.ShowDialog()
        Cursor.Current = Cursors.Default
    End Sub
    ...
    Private Sub btnArchivo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnArchivo.Click
        fnDatosAuditoria.Tipocarga = "Planilla Excel"
        fnDatosAuditoria.CentroCod = Param1_CentroCod
        ...
        lblMSG.Text = "SELECCIONE ARCHIVO EXCEL"
        pnlMSG.BackColor = Color.ForestGreen
        fnAuditoria.InsertDetalle(AuditId, Param1_CentroCod, lvALLFLEX_XLS, "Excel", "Excel", NombreSesion)
    ...
    Private Sub frmBaston_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'If EliminarDiiosBastoneo = 1 Then
        '    btnEliminaDiio.Visible = True
        'End If
        ...
        QuitarOrigenExcel = IIf(UsuarioCierraXLS = 1, False, True)
    ...
    Private Sub cboCentros_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cboCentros.SelectedIndexChanged
        ...
        Param1_CentroCod = cent_
        Param2_CentroNom = cboCentros.Text

        Cursor.Current = Cursors.WaitCursor

        cboTipoBaston.Visible = True
    End Sub

    Private Sub btnEliminaDiio_Click(sender As Object, e As EventArgs) Handles btnEliminaDiio.Click
        Dim cantidadDiios As Integer = lblTotDiios.Text
        For Each it As ListViewItem In lvXRS_DETALLE.SelectedItems
            lvXRS_DETALLE.Items.Remove(it)
        Next
        lblTotDiios.Text = cantidadDiios - 1
    End Sub
End Class